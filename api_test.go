...
func TestAPI_FetchProps(t *testing.T) {
	...
	client := newTestClient(srv.URL)
	props, err := client.fetchPropsReady(context.Background(), "test-model")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
// TestAPI_FetchModelDataRequiresLoadedModel verifies that model-scoped data
// requests establish server-authoritative readiness before fetching data.
func TestAPI_FetchModelDataRequiresLoadedModel(t *testing.T) {
	var paths []string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := client.fetchProps(context.Background(), "test-model"); err != nil {
		t.Fatalf("fetchProps failed: %v", err)
	}
	if len(paths) == 0 || paths[len(paths)-1] != "/props" {
		t.Fatalf("expected props request after readiness checks, got %v", paths)
	}
	for _, path := range paths[:len(paths)-1] {
		if path == "/props" {
			t.Fatalf("props request occurred before readiness: %v", paths)
		}
	}
}
...
func TestAPI_FetchProps_RequestError(t *testing.T) {
	client := newTestClient("http://[::1")
	if _, err := client.fetchPropsReady(context.Background(), "test-model"); err == nil {
...
func TestAPI_FetchSlots(t *testing.T) {
	...
	client := newTestClient(srv.URL)
	slots, err := client.fetchSlotsReady(context.Background(), "test-model")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(slots) != 1 {
...
func TestAPI_FetchSlots_RequestError(t *testing.T) {
	client := newTestClient("http://[::1")
	if _, err := client.fetchSlotsReady(context.Background(), "test-model"); err == nil {
...
func TestAPI_FetchMetrics(t *testing.T) {
	...
	client := newTestClient(srv.URL)
	metrics, err := client.fetchMetricsReady(context.Background(), "test-model")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestAPI_FetchMetrics_RequestError(t *testing.T) {
	client := newTestClient("http://[::1")
	if _, err := client.fetchMetricsReady(context.Background(), "test-model"); err == nil {
...
func TestAPI_FetchProps_ReadError(t *testing.T) {
	...
	_, err := client.fetchPropsReady(context.Background(), "test-model")
	if err == nil {
		t.Fatal("expected response read error")
	}
}
...
func TestAPI_FetchSlots_ReadError(t *testing.T) {
	...
	_, err := client.fetchSlotsReady(context.Background(), "test-model")
	if err == nil {
...
func TestAPI_FetchMetrics_ReadError(t *testing.T) {
	...
	_, err := client.fetchMetricsReady(context.Background(), "test-model")
	if err == nil {
		t.Fatal("expected response read error")
	}
}
