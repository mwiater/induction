...
func TestConfigValidationBranches(t *testing.T) {
	valid := Config{Server: "server", Timeout: 1, PollInterval: 1, LoadWaitInterval: 1}
...
func TestLegacyInferenceErrorPaths(t *testing.T) {
	...
	client.opts.httpClient = &http.Client{Transport: roundTripperFunc(func(req *http.Request) (*http.Response, error) {
		if strings.HasSuffix(req.URL.Path, "/models") {
			return responseWithBody(http.StatusOK, `{"data":[{"id":"model","status":{"value":"loaded"}}]}`), nil
		}
		return responseWithBody(http.StatusOK, "data: [DONE]\n"), nil
	})}
	if _, err := client.doStreamInferenceAt(context.Background(), &ChatRequest{Model: "model"}, "/completion", nil); err != nil {
		t.Fatalf("nil output should use discard: %v", err)
	}
}

func TestModelScopedSlotsAndMetricsEstablishReadiness(t *testing.T) {
	var paths []string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		paths = append(paths, r.URL.Path)
		switch r.URL.Path {
		case "/models":
			_, _ = io.WriteString(w, `{"data":[{"id":"model","status":{"value":"loaded"}}]}`)
		case "/slots":
			_, _ = io.WriteString(w, `[{"n_ctx":4096,"n_decoded":3}]`)
		case "/metrics":
			_, _ = io.WriteString(w, "# HELP ignored\nrequests_total 4\ninvalid not-a-number\n")
		default:
	...
	slots, err := client.fetchSlots(context.Background(), "model")
	if err != nil || len(slots) != 1 || slots[0]["n_decoded"] != float64(3) {
		t.Fatalf("fetchSlots = %#v, err=%v", slots, err)
	}
	metrics, err := client.fetchMetrics(context.Background(), "model")
	if err != nil || metrics.Entries["requests_total"] != float64(4) {
		t.Fatalf("fetchMetrics = %#v, err=%v", metrics, err)
	}
	if len(paths) < 4 || paths[0] != "/models" || paths[2] != "/models" || paths[len(paths)-1] != "/metrics" {
		t.Fatalf("expected readiness before each request, got paths %v", paths)
	}
}

func TestApplicationToolEntryPointAndSnapshotDecoding(t *testing.T) {
	if err := InferApplicationToolsChat(context.Background(), nil, strings.NewReader(""), io.Discard, func(context.Context, string, string) (string, error) { return "", nil }); err == nil {
		t.Fatal("expected nil request error")
	}
	if err := InferApplicationToolsChat(context.Background(), &ChatRequest{}, strings.NewReader(""), io.Discard, nil); err == nil {
		t.Fatal("expected nil handler error")
	}
	if _, err := decodeSnapshotResponse(nil); err == nil {
		t.Fatal("expected nil snapshot error")
	}
	if _, err := decodeSnapshotResponse(&ModelSnapshot{Interaction: []Interaction{{Response: "not json"}}}); err == nil {
		t.Fatal("expected malformed snapshot error")
	}

	normal, err := decodeSnapshotResponse(&ModelSnapshot{Interaction: []Interaction{{Response: `{"choices":[{"message":{"role":"assistant","content":"done"}}]}`}}})
	if err != nil || normal.Choices[0].Message.Content != "done" {
		t.Fatalf("normal snapshot = %#v, err=%v", normal, err)
	}
	stream := []InferenceStreamChunk{
		{Choices: []InferenceStreamChoice{{Index: 0, Delta: InferenceStreamDelta{Role: "assistant", Content: "answer"}}}},
		{Choices: []InferenceStreamChoice{{Index: 0, Delta: InferenceStreamDelta{Content: "!"}}}},
	}
	raw, _ := json.Marshal(stream)
	decoded, err := decodeSnapshotResponse(&ModelSnapshot{Interaction: []Interaction{{Response: string(raw)}}})
	if err != nil || decoded.Choices[0].Message.Content != "answer!" {
		t.Fatalf("stream snapshot = %#v, err=%v", decoded, err)
	}
}
