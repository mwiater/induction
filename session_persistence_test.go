...
func TestUnsavedChatSessionsAreStoredButNotListed(t *testing.T) {
	...
	if err != nil || loaded.Type != sessionTypeDirect || loaded.Saved {
...
func TestChatSessionListingSkipsInvalidAndSorts(t *testing.T) {
	directory := t.TempDir()
	first := &ChatSession{Version: chatSessionVersion, ID: "11111111-1111-4111-8111-111111111111", Type: sessionTypeDirect, Saved: true, Title: "Testing", Model: "one", CreatedAt: time.Unix(1, 0).UTC(), UpdatedAt: time.Unix(1, 0).UTC(), Messages: []Message{}}
	second := &ChatSession{Version: chatSessionVersion, ID: "22222222-2222-4222-8222-222222222222", Type: sessionTypeDirect, Saved: true, Title: "Testing", Model: "two", CreatedAt: time.Unix(2, 0).UTC(), UpdatedAt: time.Unix(2, 0).UTC(), Messages: []Message{{Role: "user", Content: "x"}}}
