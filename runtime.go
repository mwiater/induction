...
// ModelCatalogEntry is the server-authoritative model identifier and
// capability metadata returned by /v1/models.
...
// ListModelCatalog fetches model identifiers and modality capabilities
// directly from the OpenAI-compatible /v1/models endpoint.
func (c *Client) ListModelCatalog(ctx context.Context) ([]ModelCatalogEntry, error) {
	...
	result := make([]ModelCatalogEntry, 0, len(records))
	for _, record := range records {
		runtime := runtimeModelFromMap(record)
		if strings.TrimSpace(runtime.ID) == "" {
			continue
		}
		result = append(result, ModelCatalogEntry{ID: runtime.ID, Capabilities: capabilitiesFromMap(record), InputModes: runtime.InputModes, OutputModes: runtime.OutputModes})
	}
	return result, nil
}
