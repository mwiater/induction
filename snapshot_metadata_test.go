...
func TestSnapshotMetadataClassifiesMCPToolsSeparately(t *testing.T) {
	req := &ChatRequest{Messages: []Message{
		{Role: "assistant", ToolCalls: []InferenceToolCall{{ID: "call-1", Function: InferenceFunctionCall{Name: "weather", Arguments: `{"city":"London"}`}}}},
	...
	if got := snapshot.MCPToolsUsedArguments["weather"]; len(got) != 1 || got[0] != `{"city":"London"}` {
		t.Fatalf("MCP tool arguments = %#v", snapshot.MCPToolsUsedArguments)
	}
}
...
func TestSnapshotMessagesRedactsAttachmentContent(t *testing.T) {
	req := &ChatRequest{
		ImageFilename:    "/tmp/photo.png",
		DocumentFilename: "/tmp/report.pdf",
		Messages: []Message{{Role: "user", Content: []ContentPart{
			{Type: "image_url", ImageURL: &ImageURLPart{URL: "data:image/png;base64,very-large-content"}},
			{Type: "text", Text: "Document filename: report.pdf\n\nExtracted document text: very-large-content"},
		}}},
	}
	got := snapshotMessages(req)
	parts := got[0].Content.([]ContentPart)
	if parts[0].ImageURL.URL != "photo.png" || parts[1].Text != "report.pdf" {
		t.Fatalf("redacted snapshot parts = %#v", parts)
	}
	originalParts := req.Messages[0].Content.([]ContentPart)
	if originalParts[0].ImageURL.URL != "data:image/png;base64,very-large-content" {
		t.Fatalf("request image data was modified: %#v", originalParts[0].ImageURL.URL)
	}
}
