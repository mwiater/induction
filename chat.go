...
func startChatMonitor(ctx context.Context, req *ChatRequest, options ...ClientOption) (*Client, ChatRequest, *inferenceMonitor, time.Duration, error) {
	...
	// GenerateSnapshot/GenerateStreamingSnapshot performs the authoritative
	// model-load gate immediately before inference. Loading here as well would
	// issue a duplicate switch through a separate client in noninteractive chat
	// and can race a server's model transition.
