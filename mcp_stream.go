...
func runMCPStreamToolLoop(ctx context.Context, req *ChatRequest, out io.Writer, tools []boundMCPTool, timeout time.Duration, approve MCPApprovalFunc, options ...ClientOption) error {
	_, err := runMCPStreamToolLoopResult(ctx, req, out, tools, timeout, approve, options...)
	return err
}

func runMCPStreamToolLoopResult(ctx context.Context, req *ChatRequest, out io.Writer, tools []boundMCPTool, timeout time.Duration, approve MCPApprovalFunc, options ...ClientOption) (*InferenceResponseMessage, error) {
	...
	for turn := 0; turn < maxMCPToolTurns; turn++ {
		...
		if err != nil {
			return nil, fmt.Errorf("stream inference with MCP tools: %w", err)
		}
		...
		for _, call := range assistant.ToolCalls {
			...
			if !json.Valid(args) {
				return nil, fmt.Errorf("model returned malformed arguments for MCP tool %q: %q", binding.tool.Name, call.Function.Arguments)
			}
