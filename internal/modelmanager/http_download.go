...
func DownloadHTTP(ctx context.Context, client *http.Client, request DownloadRequest, progress ProgressFunc) (Manifest, error) {
	...
	completed, err := streamHubFile(ctx, client, url, staging, request.Size, request.Token, progress)
	if err != nil {
		return Manifest{}, err
	}
...
func DownloadMultiHTTP(ctx context.Context, client *http.Client, modelsPath, repository, revision string, files []ModelFile, overwrite bool, progress ProgressFunc, configuredToken ...string) (Manifest, string, error) {
	...
	if len(configuredToken) > 0 {
		token = configuredToken[0]
	}
	for _, file := range set {
		installed, err := DownloadHTTP(ctx, client, DownloadRequest{Repository: repository, File: file.Path, Revision: revision, ModelsPath: modelsPath, Size: file.Size, ETag: file.ETag, LFSOID: file.LFSOID, Overwrite: overwrite, Token: token}, progress)
		if err != nil {
			return Manifest{}, "", err
		}
...
func streamHubFile(ctx context.Context, client *http.Client, url, staging string, expected int64, token string, progress ProgressFunc) (int64, error) {
	if client == nil {
