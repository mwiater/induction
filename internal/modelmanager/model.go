...
func (m *Model) downloadCmd() tea.Cmd {
	...
	return func() tea.Msg {
		...
		if len(shards) > 1 {
			manifest, path, err := DownloadMulti(ctx, options.HFPath, options.ModelsPath, repository, revision, shards, options.HuggingFaceToken)
			return downloadCompletedMsg{manifest, path, err}
		}
		manifest, err := Download(ctx, options.HFPath, DownloadRequest{Repository: repository, File: file.Path, Revision: revision, ModelsPath: options.ModelsPath, Size: file.Size, ETag: file.ETag, LFSOID: file.LFSOID, Overwrite: overwrite, Token: options.HuggingFaceToken})
...
func (m *Model) startProgressDownload(shards []ModelFile) tea.Cmd {
	...
	m.transfer = events
	request := DownloadRequest{Repository: m.selectedRepository.ID, File: m.selectedFile.Path, Revision: m.revision, ModelsPath: m.options.ModelsPath, Size: m.selectedFile.Size, ETag: m.selectedFile.ETag, LFSOID: m.selectedFile.LFSOID, Overwrite: m.overwrite, Token: m.options.HuggingFaceToken}
	...
	go func() {
		...
		var err error
		if len(shards) > 1 {
			manifest, manifestPath, err = DownloadMultiHTTP(ctx, nil, modelsPath, repository, revision, shards, overwrite, callback, request.Token)
		} else {
