...
func NewRootCommand() *cobra.Command {
	...
	root := &cobra.Command{Use: "induction", Short: "run inference and manage Induction", SilenceUsage: true, SilenceErrors: true,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if inference.model == "" && inference.pipeline == "" && inference.image == "" && inference.document == "" && inference.documents == "" && inference.userPrompt == "" && inference.systemPrompt == "" && inference.responseFormat == "" && inference.jsonSchema == "" && !inference.parameterOverrides {
				return cmd.Help()
			}
	...
	root.AddCommand(newPipelineCommand(&configPath))
...
// requiring an explicit --beta acknowledgement. Each command gets a local
// flag so Cobra enforces the requirement whether the user invokes a group or
// one of its subcommands (for example, "models --beta" or
// "models list --beta").
func markBetaCommandTree(cmd *cobra.Command) {
	cmd.Flags().Bool("beta", false, "acknowledge that this feature is in development")
	if err := cmd.MarkFlagRequired("beta"); err != nil {
		panic(fmt.Sprintf("mark %s as beta: %v", cmd.CommandPath(), err))
	}
	for _, child := range cmd.Commands() {
		markBetaCommandTree(child)
	}
}
...
func newListCommand(root *cobra.Command) *cobra.Command {
	commands := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			out := cmd.OutOrStdout()
			_, _ = fmt.Fprintln(out, "Commands and Subcommands:")
			for _, data := range commandData {
				if strings.Contains(data.path, "completion") {
					continue
				}
...
func renderModelInspection(out io.Writer, result *induction.ModelInspection) error {
	...
	if r.ContextSize != nil {
		...
		_, _ = fmt.Fprintln(w, "  Context\tunknown")
	}
	if r.BatchSize != nil {
		_, _ = fmt.Fprintf(w, "  Batch\t%d\n", *r.BatchSize)
	} else {
		_, _ = fmt.Fprintln(w, "  Batch\tunknown")
	}
	if r.UBatchSize != nil {
		_, _ = fmt.Fprintf(w, "  UBatch\t%d\n", *r.UBatchSize)
	} else {
		_, _ = fmt.Fprintln(w, "  UBatch\tunknown")
	}
	if r.Parallel != nil {
		_, _ = fmt.Fprintf(w, "  Parallel\t%d\n", *r.Parallel)
	} else {
		_, _ = fmt.Fprintln(w, "  Parallel\tunknown")
	}
	_, _ = fmt.Fprintf(w, "  Cache K\t%s\n  Cache V\t%s\n", valueOrUnknown(r.CacheTypeK), valueOrUnknown(r.CacheTypeV))
	if r.FlashAttention == nil {
		_, _ = fmt.Fprintln(w, "  Flash Attn\tunknown")
	} else {
		_, _ = fmt.Fprintf(w, "  Flash Attn\t%s\n", boolLabel(*r.FlashAttention))
	}
	return w.Flush()
}
...
func newInstalledCommands(configPath *string, inherited func() (string, int, []string)) []*cobra.Command {
	...
	listCmd := &cobra.Command{Use: "list", Short: "list installed or loaded models", RunE: func(cmd *cobra.Command, _ []string) error {
		...
		if loaded {
			...
			if len(names) == 0 {
				_, _ = fmt.Fprintln(cmd.OutOrStdout(), "No models currently loaded.")
				return nil
			}
		...
		for _, item := range index.Installations {
			_, _ = fmt.Fprintln(cmd.OutOrStdout(), item.Manifest.RepositoryID+"/"+item.Manifest.ModelFile)
		}
		return nil
	}}
	...
	verifyCmd := &cobra.Command{Use: "verify [MODEL]", Short: "verify an installed model", Args: cobra.MaximumNArgs(1), RunE: func(cmd *cobra.Command, args []string) error {
		...
		_, _ = fmt.Fprintln(cmd.OutOrStdout(), result.Status)
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, result := range results {
				_, _ = fmt.Fprintln(cmd.OutOrStdout(), result.ID)
			}
			return nil
		},
	}
