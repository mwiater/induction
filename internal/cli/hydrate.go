...
func runHydrate(cmd *cobra.Command, configPath string) error {
	...
	if len(catalog) == 0 {
		_, _ = fmt.Fprintln(cmd.OutOrStdout(), "No models were reported by /v1/models; generating dashboard from existing sessions.")
	} else {
		_, _ = fmt.Fprintf(cmd.OutOrStdout(), "Hydrating %d server-reported model(s).\n", len(catalog))
	}

	mcpConfigured := len(cfg.MCPServers) > 0
	if !mcpConfigured {
		_, _ = fmt.Fprintln(cmd.OutOrStdout(), "MCP workload skipped: no MCP servers are configured.")
	}
	...
	for _, entry := range catalog {
		...
		if !vision {
			summary.Skipped++
			_, _ = fmt.Fprintf(cmd.OutOrStdout(), "[%s] image workload skipped: model does not report image input.\n", model)
		}
	}

	_, _ = fmt.Fprintln(cmd.OutOrStdout(), "Running dashboard generation…")
	...
	_, _ = fmt.Fprintf(cmd.OutOrStdout(), "Hydration complete: models=%d scheduled=%d succeeded=%d failed=%d image_workloads_skipped=%d\n", summary.Models, summary.Scheduled, summary.Succeeded, summary.Failed, summary.Skipped)
	return nil
}
