...
`)
	...
	if !client.opts.liveMetricsOverlayEnabled {
...
func TestConfigValidatesMCPServers(t *testing.T) {
	valid := func() Config {
		return Config{Server: "http://localhost:9998", Timeout: Duration(time.Second), PollInterval: Duration(time.Second), LoadWaitInterval: Duration(time.Second)}
	}
	tests := []struct {
		name    string
