...
func TestInferSnapshotUsesConfigAndRequestModel(t *testing.T) {
	var inferenceRequests []ChatRequest
	...
	httpClient := &http.Client{Transport: roundTripperFunc(func(req *http.Request) (*http.Response, error) {
		...
		case "/v1/chat/completions":
			...
			inferenceRequests = append(inferenceRequests, request)
	...
	options := []ClientOption{WithHTTPClient(httpClient), WithLiveMetricsOverlay(false)}
	snapshot, err := InferSnapshot(context.Background(), &ChatRequest{
		Model:    "test-model",
		Messages: []Message{{Role: "user", Content: "hello"}},
	}, options...)
	if err != nil {
		t.Fatalf("InferSnapshot failed: %v", err)
	}
	if snapshot.ModelID != "test-model" || len(inferenceRequests) != 1 || inferenceRequests[0].Model != "test-model" {
		t.Fatalf("request model was not applied: snapshot=%#v requests=%#v", snapshot, inferenceRequests)
	}
	if !sawDeadline.Load() {
		t.Fatal("configured timeout was not applied to requests")
	}

	_, err = InferSnapshot(context.Background(), &ChatRequest{
		Model:    "override-model",
		Messages: []Message{{Role: "user", Content: "hello"}},
	}, options...)
	if err != nil {
		t.Fatalf("InferSnapshot with model override failed: %v", err)
	}
	if len(inferenceRequests) != 2 || inferenceRequests[1].Model != "override-model" {
		t.Fatalf("request model override was not preserved: %#v", inferenceRequests)
	}
}

func TestInferSnapshotRejectsNilRequest(t *testing.T) {
	if _, err := InferSnapshot(context.Background(), nil); err == nil {
		t.Fatal("expected nil request error")
	}
}

func TestInferSnapshotRequiresRequestModel(t *testing.T) {
	if _, err := InferSnapshot(context.Background(), &ChatRequest{}); err == nil {
