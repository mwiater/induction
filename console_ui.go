...
type consoleModel struct {
	...
	footerBarPipelineStyle      lipgloss.Style
...
func newConsoleModel(ctx context.Context, client *Client, request ChatRequest, timeout time.Duration, sidebarWidth int, mode consoleMode) consoleModel {
	...
	m := consoleModel{
		...
		footerBarPipelineStyle:      defaultConsoleUITheme.footerBarPipeline,
...
func (m consoleModel) Init() tea.Cmd {
	loadModel := func() tea.Msg {
		...
		defer cancel()
		_, err := m.client.ensureModelLoaded(loadCtx, m.request.Model)
		if err != nil {
			return consoleModelLoadedMsg{err: err}
		}
		props, _ := m.client.fetchPropsReady(loadCtx, m.request.Model)
		slots, _ := m.client.fetchSlotsReady(loadCtx, m.request.Model)
...
func (m consoleModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case consoleModelLoadedMsg:
		if msg.err != nil {
			m.client.logf("ui: initial model load failed model=%q error=%v", m.request.Model, msg.err)
		...
		m.loading = false
		m.client.logf("ui: initial model ready model=%q", m.request.Model)
		m.props = msg.props
		m.slots = msg.slots
		...
		m.input.Focus()
		if m.pipeline != nil {
			return m, m.startPipelineStep(0)
		}
	...
	case consoleOverlayMsg:
		...
		expectedModel := m.request.Model
		if m.modelLoading != "" {
			expectedModel = m.modelLoading
		}
		if update.model != "" && update.model != expectedModel {
			return m, nil
		}
		if update.footer != "" {
			m.footer = update.footer
		}
		if update.modelReady && m.modelLoading == "" && !m.waiting {
			m.loading = false
			m.input.SetValue("")
			m.input.Prompt = " " + renderWhiteIcon(DefaultUnicode.Star) + " You: "
			if m.inputReady() {
	...
	case consoleTurnResult:
		...
		if m.pipeline != nil && m.pipelineStep >= 0 && m.pipelineStep < len(m.pipeline.Steps) {
			...
			if step.ResponseFormat != nil {
				...
				if (typeName == "json" || typeName == "json_object" || typeName == "json_schema") && !isJSONObject(msg.content) {
					m.client.logf("pipeline: step %d/%d returned invalid JSON object", m.pipelineStep+1, len(m.pipeline.Steps))
					m.pipelineRunning = false
					m.err = fmt.Errorf("pipeline step %q returned invalid JSON object", step.Name)
					m.refreshTranscript()
					return m, nil
				}
			}
		...
		if m.session != nil && msg.snapshot != nil {
			...
			updateSessionFinalOutput(m.session)
		}
		if msg.snapshot != nil {
		...
		if m.session != nil {
			...
			m.input.Blur()
			cmds = append(cmds, saveSessionCmd(m.client, m.session, m.autoExitAfterInitialChat && m.pipeline == nil))
		} else {
			m.input.Focus()
			if m.autoExitAfterInitialChat {
	...
	case sessionSavedMsg:
		...
		if msg.err != nil {
			m.client.logf("session: save failed error=%v", msg.err)
		...
		if !msg.session.Saved {
			...
			m.sessionStatus = "Saved: " + msg.session.Title + " (" + msg.session.ID + ")"
		}
		if m.pipeline != nil && m.pipelineRunning {
			if m.pipelineStep+1 >= len(m.pipeline.Steps) {
				m.pipelineRunning = false
				return m, tea.Quit
			}
			m.pipelineStep++
	...
	case modelLoadedMsg:
		...
		m.modelLoading = ""
		if msg.err != nil {
			m.client.logf("ui: model switch failed model=%q error=%v", msg.model, msg.err)
			m.pipelineRunning = false
		...
		if m.modelMonitor != nil && m.modelMonitor != msg.monitor {
			m.modelMonitor.Stop()
		}
		m.modelMonitor = msg.monitor
		if m.modelMonitor != nil {
	...
	case tea.KeyMsg:
		...
		if m.sessionView == sessionViewSave {
			...
			case "enter":
				...
				if m.session != nil {
					...
					session = &updated
					session.Saved = true
					configureSessionMode(session, m.pipeline)
					session.Title = title
					if m.pipeline != nil {
						session.Title = m.pipeline.Name
					}
				...
				m.sessionLoading = true
				return m, saveSessionCmd(m.client, session, false)
			}
		...
		if m.input.Focused() && m.inputReady() {
			switch msg.String() {
			case "enter":
...
func (m *consoleModel) startPipelineStep(index int) tea.Cmd {
	if m.pipeline == nil || index < 0 || index >= len(m.pipeline.Steps) {
		return nil
	}
	...
	if m.request.Model == step.Model {
		return m.submitPipelineStep(index)
	}
	m.loading = true
	m.input.Blur()
	m.modelLoading = step.Model
	if m.modelOverlay != nil {
		m.modelOverlay.StartModelLoad(step.Model)
	}
	m.input.Prompt = fmt.Sprintf(" %s You: (model %s is loading, please wait...)", renderWhiteIcon(DefaultUnicode.Star), step.Model)
...
func (m *consoleModel) submitPipelineStep(index int) tea.Cmd {
	...
	// Output constraints belong to the current step. Clear them first so a
	...
	if step.Parameters != nil {
		m.request.Temperature = step.Parameters.Temperature
		m.request.TopP = step.Parameters.TopP
		m.request.TopK = step.Parameters.TopK
	...
	if step.Image != "" {
		...
		content = []ContentPart{{Type: "text", Text: step.UserPrompt}, {Type: "image_url", ImageURL: &ImageURLPart{URL: dataURL, Detail: "low"}}}
		...
		content = []ContentPart{{Type: "text", Text: step.UserPrompt}, {Type: "text", Text: "Document filename: " + filename + "\n\nExtracted document text:\n" + documentText}}
	}
	m.request.Messages = append(m.request.Messages, Message{Role: "user", Content: content})
	m.messages = append(m.messages, consoleMessage{role: "user", content: step.UserPrompt})
	m.waiting = true
	if m.modelOverlay != nil {
		m.modelOverlay.StartInference(step.Model)
	}
	m.sessionDirty = true
	m.input.Blur()
	m.sessionStatus = fmt.Sprintf("Pipeline step %d/%d: %s", index+1, len(m.pipeline.Steps), step.Name)
	m.refreshTranscript()
	return m.runTurn(m.request)
}

// addPipelineSystemPrompt keeps pipeline instructions compatible with chat
// templates that allow exactly one system message at the beginning.
func addPipelineSystemPrompt(messages []Message, prompt string) []Message {
	...
	for leading < len(messages) && messages[leading].Role == "system" {
		if existing, ok := messages[leading].Content.(string); ok && strings.TrimSpace(existing) != "" {
			instructions = append(instructions, strings.TrimSpace(existing))
		}
		leading++
	}
	instructions = append(instructions, "Pipeline step instructions:\n"+prompt)
	result := make([]Message, 0, len(messages)-leading+1)
	result = append(result, Message{Role: "system", Content: strings.Join(instructions, "\n\n")})
	result = append(result, messages[leading:]...)
	return result
}
...
func (m *consoleModel) submitInput() tea.Cmd {
	...
	m.input.Blur()
	m.err = nil
	m.client.logf("ui: submitting user turn model=%q prompt=%q", m.request.Model, value)
	m.waiting = true
	if m.modelOverlay != nil {
		m.modelOverlay.StartInference(m.request.Model)
	}
	m.messages = append(m.messages, consoleMessage{role: "user", content: value})
	if !replaceLastMultimodalUserPrompt(&m.request, value) {
...
// inputReady is true only while the loaded model is idle and the chat view is
// waiting for a new user turn. Inference stages such as encoding and decoding
// are represented by waiting, so the input remains unavailable throughout the
// complete request lifecycle.
func (m consoleModel) inputReady() bool {
	return m.sessionView == sessionViewChat && !m.loading && !m.waiting && !m.sessionLoading && m.modelLoading == ""
}

// replaceLastMultimodalUserPrompt keeps an automated prompt attached to an
// image or document instead of creating a separate text-only turn.
func replaceLastMultimodalUserPrompt(req *ChatRequest, prompt string) bool {
	...
	message.Content = append(parts, ContentPart{Type: "text", Text: prompt})
	return true
}
...
func (m *consoleModel) syncSessionFromRequest() {
	if m.session != nil {
		...
		m.session.imageFilename = m.request.ImageFilename
		m.session.documentFilename = m.request.DocumentFilename
	}
}
...
func saveSessionCmd(client *Client, session *ChatSession, exit bool) tea.Cmd {
	copy := *session
...
func loadModelCmd(client *Client, ctx context.Context, timeout time.Duration, model, previous string, overlay *liveMetricsOverlay) tea.Cmd {
	return func() tea.Msg {
		...
		defer cancel()
		client.logf("ui: loading model model=%q previous=%q", model, previous)
		var err error
		if strings.TrimSpace(previous) != "" && previous != model {
			client.logf("runtime: switching model previous=%q target=%q", previous, model)
			_, err = client.SwitchModel(loadCtx, model)
		} else {
		...
		if err == nil && overlay != nil {
			overlay.modelLoadedForModel(model)
		}
		var props *PropsData
		var slots SlotsData
		if err == nil {
			props, _ = client.fetchPropsReady(loadCtx, model)
			slots, _ = client.fetchSlotsReady(loadCtx, model)
			if monitor != nil {
...
func loadSessionCmd(summary ChatSessionSummary, client *Client, ctx context.Context, timeout time.Duration) tea.Cmd {
	return func() tea.Msg {
		session, err := loadChatSessionFromPath(summary.Path)
		if err != nil {
...
// sendConsoleInferenceChunk renders both separately returned reasoning and
// assistant content as one transcript stream. The same helper is used by
// ordinary, application-tool, and MCP-backed UI inferences.
func (m consoleModel) sendConsoleInferenceChunk(chunk InferenceStreamChunk, reasoningOpen *bool) {
...
func (m consoleModel) closeConsoleInferenceReasoning(reasoningOpen *bool) {
	...
	return func() tea.Msg {
		...
		if m.client != nil && m.client.opts != nil && m.client.opts.applicationToolHandler != nil {
			response, snapshot, err := runApplicationToolLoopWithObserver(turnCtx, &request, m.client.opts.applicationToolHandler, m.client.opts.applicationToolChain, func(inferCtx context.Context, turn *ChatRequest) (*InferenceResponse, *ModelSnapshot, error) {
				reasoningOpen := false
				snapshot, err := m.client.withoutLiveMetricsOverlay(inferCtx).GenerateStreamingSnapshot(inferCtx, turn, func(chunk InferenceStreamChunk) error {
					m.sendConsoleInferenceChunk(chunk, &reasoningOpen)
					return nil
				})
				m.closeConsoleInferenceReasoning(&reasoningOpen)
				if err != nil {
					return nil, snapshot, err
				}
				response, err := decodeSnapshotResponse(snapshot)
				return response, snapshot, err
			}, func(call InferenceToolCall) {
				m.client.logf("tool call: type=application name=%q arguments=%s", call.Function.Name, call.Function.Arguments)
				if m.send != nil {
					m.send(consoleToolMsg(fmt.Sprintf("[Tool Call] %s(%s)", call.Function.Name, call.Function.Arguments)))
				}
			})
		...
		case consoleStreaming:
			m.client.logf("ui: streaming inference started model=%q messages=%d", request.Model, len(request.Messages))
			reasoningOpen := false
...
func (m *consoleModel) resize() {
	...
	// The main view has four fixed rows: the header, User Input header,
	// input line, and trailing spacer. Do not force a transcript row when the
	// terminal is too short; overflow here pushes stale-looking input and
	// footer rows into the next render region.
	m.transcript.Height = max(0, contentHeight-4)
...
func (m consoleModel) footerHeight() int {
	height := consoleFooterHeight + 1 // live metrics and key bindings
	if m.mcpFooter != "" {
		height++
	}
	if m.pipelineRunning && m.pipeline != nil {
		height++
	}
	return height
}
...
func (m *consoleModel) refreshTranscript() {
	var out strings.Builder
	messageWidth := max(1, m.transcript.Width-2) // reserve the message icon
	assistantContentStyle := m.mainAssistantContentStyle.Width(messageWidth)
	assistantReasoningStyle := m.mainAssistantReasoningStyle.Width(messageWidth)
	for i, message := range m.messages {
		...
		if message.role == "assistant" {
			out.WriteString(renderAssistantMessage(assistantReasoningStyle, assistantContentStyle, icon, label, message.content))
		} else {
			out.WriteString(" " + renderWhiteIcon(icon) + style.Width(messageWidth).Render(label+indentContinuationLines(message.content)))
		}
	}
	if m.err != nil {
		if out.Len() > 0 {
...
func (m *consoleModel) refreshSidebar() {
	...
	// Slot and props data describe server/model defaults. Explicit request
	// parameters take precedence in the sidebar so the UI shows the values
	// that will be sent for this chat, rather than only the loaded defaults.
	if m.request.Temperature != nil {
		fields["temperature"] = *m.request.Temperature
	}
	if m.request.TopP != nil {
		fields["top_p"] = *m.request.TopP
	}
	if m.request.TopK != nil {
		fields["top_k"] = *m.request.TopK
	}
	if m.request.MaxTokens != nil {
		fields["max_tokens"] = *m.request.MaxTokens
	}
	if m.request.RepeatPenalty != nil {
		fields["repeat_penalty"] = *m.request.RepeatPenalty
	}
	if m.request.Seed != nil {
		fields["seed"] = *m.request.Seed
	}
...
func (m consoleModel) View() string {
	...
	if m.session != nil && strings.TrimSpace(m.session.ID) != "" {
		chatSessionTitle = fmt.Sprintf("Chat Session (%s)", chatSessionPath(m.session.ID))
	}
	main := strings.Join([]string{
		renderFullWidthHeader(DefaultUnicode.Home, chatSessionTitle, defaultConsoleUITheme.mainHeader, m.transcript.Width),
		m.transcript.View(),
	...
	footerText := singleConsoleLine(m.footer)
	// Pipeline progress has its own sticky footer. Keeping it out of the live
	// metrics line prevents the same status from being rendered twice.
	if m.sessionStatus != "" && m.pipeline == nil {
		footerText += " | " + singleConsoleLine(m.sessionStatus)
	}
	...
	if m.mcpFooter != "" {
		mcpFooter := ansi.Truncate(singleConsoleLine(m.mcpFooter), max(0, m.width-1), "")
	...
	if m.pipelineRunning && m.pipeline != nil && m.pipelineStep >= 0 && m.pipelineStep < len(m.pipeline.Steps) {
		pipelineFile := filepath.Base(m.pipeline.FilePath)
		if pipelineFile == "." || pipelineFile == string(filepath.Separator) || pipelineFile == "" {
			pipelineFile = m.pipeline.Name
		}
		step := m.pipeline.Steps[m.pipelineStep]
		pipelineText := fmt.Sprintf("%s | Pipeline step %d/%d: %s", pipelineFile, m.pipelineStep+1, len(m.pipeline.Steps), step.Name)
		pipelineFooter := renderIconBar(DefaultUnicode.DoubleArrowRight, "#FFFF00", ansi.Truncate(singleConsoleLine(pipelineText), max(0, m.width-1), ""), m.footerBarPipelineStyle.Width(m.width).MaxHeight(1))
		footer = lipgloss.JoinVertical(lipgloss.Left, footer, pipelineFooter)
	}
...
// singleConsoleLine prevents dynamic status text from changing the number of
// rows owned by the footer. Newlines can otherwise make Bubble Tea's frame
// shorter or taller than the previous frame and expose stale terminal rows.
func singleConsoleLine(value string) string {
...
func pipelineStepCount(pipeline *Pipeline) int {
	if pipeline == nil {
		return 0
	}
...
func runConsoleMCPChat(ctx context.Context, req *ChatRequest, in io.Reader, out io.Writer, approve MCPApprovalFunc, options ...ClientOption) error {
	...
	status := func(message string) {
		client.logf("tool call: type=mcp status=%q", strings.TrimSpace(message))
		if program != nil {
	...
	model.inferSnapshotTurn = func(turnCtx context.Context, turn *ChatRequest) (*InferenceResponse, *ModelSnapshot, error) {
		...
		if model.pipeline != nil && pipelineStep >= 0 && pipelineStep < len(model.pipeline.Steps) && model.pipeline.Steps[pipelineStep].NoMCP {
			...
			snapshot, err := client.withoutLiveMetricsOverlay(turnCtx).GenerateStreamingSnapshot(turnCtx, turn, func(chunk InferenceStreamChunk) error {
				model.sendConsoleInferenceChunk(chunk, &reasoningOpen)
				return nil
			})
			model.closeConsoleInferenceReasoning(&reasoningOpen)
			if err != nil {
				return nil, snapshot, err
			}
			response, err := decodeSnapshotResponse(snapshot)
		...
		response, err := runMCPToolLoopWith(turnCtx, turn, tools, timeout, approve, status, func(inferCtx context.Context, toolTurn *ChatRequest) (*InferenceResponse, error) {
			reasoningOpen := false
			snapshot, err := client.withoutLiveMetricsOverlay(inferCtx).GenerateStreamingSnapshot(inferCtx, toolTurn, func(chunk InferenceStreamChunk) error {
				model.sendConsoleInferenceChunk(chunk, &reasoningOpen)
				return nil
			})
			model.closeConsoleInferenceReasoning(&reasoningOpen)
...
func formatMCPToolActivity(message string, tools []boundMCPTool) (string, bool) {
	...
	toolName := descriptor
	arguments := ""
	if open := strings.IndexByte(descriptor, '('); open >= 0 && strings.HasSuffix(descriptor, ")") {
		toolName = descriptor[:open]
		arguments = descriptor[open+1 : len(descriptor)-1]
	}
	...
	return fmt.Sprintf("   [Tool Call: %s] %s: %s(%s)", label, server, toolName, arguments), true
}
